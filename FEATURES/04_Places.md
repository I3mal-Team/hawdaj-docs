...
- الترتيب: بموقع → مسافة Haversine ثم order_id/id؛ `top_visited` → views_num DESC؛ وإلا order_id/id. **الصفوف بلا إحداثيات (distance=NULL) تُدفَع للنهاية** (`orderByRaw('distance IS NULL')`, e3cab23) — يشمل places/home/map/related/search وكل الكيانات التابعة (05/06/09/10/11) لأن الإصلاح في `applyDistanceSorting` المشترك.
