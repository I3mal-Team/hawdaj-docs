...
## 3. Business Rules
- المحتوى عام (بلا مصادقة). **بموقع → حساب طازج بلا cache** (ترتيب يتبع الموضع الحيّ)؛ **بلا موقع → cache عام 10 دقائق** مُبطَل فوريًا عبر version bump (راجع §24). *(محدَّث 2026-07-08 — e3cab23)*
- الترتيب: إن توفّر موقع (lat/lng) → ترتيب بالمسافة (Haversine)، وإلا → `order_id` ثم `id` (وبعض الأقسام `views_num`). **الصفوف بلا إحداثيات (distance=NULL) تُدفَع للنهاية** عبر `orderByRaw('distance IS NULL')` (كانت تتصدّر بسبب ترتيب MySQL لـNULL أولًا في ASC).
...
|---|---|---|---|
| GET | `home` | ❌ عام | **بموقع: بلا cache** · بلا موقع: 10د (مفتاح `home_data:v{N}:...` + version bump) |
| GET | `sliders` | ❌ عام | بلا cache على الـ controller |
Query params لـ home: `lat`, `lng`, `search`, و`*_per_page` لكل قسم.

**حلّ الموقع (`getLocationCoords`, `App.php:877`) بأولوية:** (1) `lat`/`lng` على الطلب (الأعلى) → (2) مستخدم مُوثّق: cache `user_location_user_{id}` ثم جدول `user_locations` (TTL 30 يوم، DB-backed) → (3) fallback IP: `user_location_ip_{md5(ip)}` من آخر `update-location` (TTL 6 ساعات، ضيف). فالموبايل لا يحتاج تمرير `lat/lng` صراحةً لو نفّذ `update-location` (يُخزَّن ويُحلّ لاحقًا).
...
## 16. Controllers
- **HomeController** (`app/Http/Controllers/Api/HomeController.php`): `getHome` (تسجيل زيارة → `getLocationCoords`؛ **بموقع: `fetchHomeData` مباشرة بلا cache**؛ بلا موقع: `Cache::remember('home_data:v{version}:...', 600)`) → `fetchHomeData` (11 قسمًا، eager loading، ترتيب بالمسافة/القائمة). مفتاح الـcache يحمل `homeCacheVersion()` يُرفَع بـ`bumpHomeCacheVersion()`.
...
- مفتاح الـcache (بلا موقع فقط) يتضمّن locale، فتغيير اللغة لا يخلط الترجمات.
- ✅ **مُصلَح (e3cab23):** المحتوى الجديد/المعدّل يظهر فورًا — `PlaceObserver::saved()/deleted()` على Place/Event/Store/ZadElgadel يستدعي `bumpHomeCacheVersion()` فيُبطِل كل payloads home المخزّنة لحظيًا (بدل انتظار TTL). *(cache ملفّي بلا tags → الإبطال عبر version token في المفتاح.)*
- ✅ **مُصلَح (e3cab23) — تلوّث cache بالموقع:** الطلبات ذات الموقع تتخطّى الـcache كليًا (لا bucket ~1كم)، فلا يرث مستخدم ترتيب موقع مستخدم آخر. الـcache يبقى لمسار «بلا موقع» فقط (`geoBucket='noloc'`).
- ⚠️ باقٍ: حالة المفضّل لا تُخزَّن في payload (فحص isFavorite وقت الرسم فقط) — غير متأثّر بالتعديل.
...
HomeCubit.fetchHome() (main.dart)  →  emit(Loading)
  ...
  → visit() log · getLocationCoords → (بموقع: fetchHomeData مباشرة | بلا موقع: Cache::remember('home_data:v{N}:...',600))
...
- ✅ **مُحسَّن (e3cab23):** مفتاح الـcache صار يحمل locale + version؛ ومسار الموقع خارج الـcache. بقيت مقايضة: الطلبات ذات الموقع تحسب طازجًا دائمًا (~40–50 استعلام) بلا cache — حمل أعلى لكن دقّة ترتيب حيّة.
...
- ✅ **مُعالَج (e3cab23) — cache poisoning بالموقع:** الطلبات ذات الموقع لم تعد تكتب/تقرأ cache مشترك؛ لا يتسرّب ترتيب موقع مستخدم لآخر.
...
- ✅ التحديث البطيء (حتى ساعة) عولِج بـversion bump (e3cab23) — يظهر فورًا الآن.

## 31. Improvement Opportunities
- ✅ منفَّذ (e3cab23): locale + version في المفتاح، والترتيب الجغرافي خارج الـcache. باقٍ: cache قصير (bucket geohash) للطلبات ذات الموقع لتخفيف الحمل مع حدّ تلوّث مقبول.
...
- ✅ منفَّذ (e3cab23): locale + version في المفتاح + ترتيب الموقع خارج الـcache (إزالة التلوّث).
