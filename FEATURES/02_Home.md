...
|---|---|---|---|
| GET | `home` | ❌ عام | 10 دقائق، **مفتاح مُبطّن `home_data:{locale}:{lat_lng ~1km|noloc}:{md5(params)}`** (كان: مفتاح عام `home_data` 60د — أُصلح 2026-07-08) |
| GET | `sliders` | ❌ عام | بلا cache على الـ controller |
Query params لـ home: `lat`, `lng`, `search`, و`*_per_page` لكل قسم.

## 13. Request Models
لا request body (GET). المدخلات query params اختيارية: `lat`, `lng`، `search`، أحجام الصفحات لكل قسم. **(2026-07-08)** الموبايل يمرّر `lat`/`lng` فعليًا الآن: `HomeCubit.fetchHome({lat,lng})` → `HomeRepoImp` يضيفهما لـ`queryParameters` (fallback على `LocationCache`)؛ ويعيد الجلب عبر `BlocListener<LocationCubit>` عند `LocationLoaded`. (سابقًا: `get(EndPoints.home)` عارٍ بلا موقع، وLocationCubit غير مُستهلَك.)
...
- ✅ **(أُصلح 2026-07-08) cache مُبطّن بالموقع/اللغة + TTL 10د**: كان مفتاح عام 60د → محتوى جديد يظهر أسرع (10د بدل 60د). لا invalidation بعد، فالتقادم محدود بالـTTL فقط.
- ✅ **(أُصلح 2026-07-08) تلوّث cache بالموقع زال:** `getLocationCoords()` نُقل خارج الـclosure والمفتاح صار يشمل الموقع المبطّن (~1km) + اللغة → كل مستخدم/لغة يرى ترتيبه الصحيح. راجع §35.
...
- ✅ **(أُصلح 2026-07-08)** المفتاح صار يضمّن الموقع المبطّن + اللغة + params (page/‏*_per_page/search) عبر `HomeController::homeCacheKey()` → لا نتائج مضلّلة. `getLocationCoords()` يُحسب مرة خارج الـclosure ويُحقن في `fetchHomeData($request,$location)`.
...
- ✅ **(أُصلح 2026-07-08) cache poisoning بالموقع زال:** المفتاح صار per-location bucket + per-locale، فلا يؤثر موقع مستخدم على غيره.
...
- ~~المفتاح العام يجعل التحديث بطيء الظهور (حتى ساعة)~~ → ✅ TTL خُفّض لـ10د + المفتاح مُبطّن (2026-07-08). يبقى: لا cache invalidation عند تعديل المحتوى (file driver بلا tags) — تقادم أقصاه 10د.
...
| العطل | السبب | ابدأ من |
|---|---|---|
| «محتوى قديم رغم التحديث» | ~~cache عام 60د~~ → الآن TTL 10د (لا invalidation بعد) | `HomeController@getHome` (Cache::remember) |
| ~~«ترتيب غريب للأماكن»~~ ✅ محلول 2026-07-08 | كان تلوّث cache بموقع آخر + الموبايل لا يبعت lat/lng | المفتاح المبطّن `homeCacheKey()` + الموبايل يمرّر lat/lng ويعيد الجلب عند `LocationLoaded` |
