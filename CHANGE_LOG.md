...
## 2026-07-08
- `feat` · [[02_Home]] [[04_Places]] · **ترتيب الأماكن بالأقرب لموقع الجهاز + تحديث فوري (backend, `e3cab23`).** (1) `applyDistanceSorting`: `orderByRaw('distance IS NULL')` يدفع الصفوف بلا إحداثيات للنهاية (كانت تتصدّر — MySQL يرتّب NULL أولًا ASC) عبر places/home/map/related/search. (2) `HomeController@getHome`: بموقع → `fetchHomeData` مباشرة بلا cache (ترتيب حيّ بإحداثيات دقيقة، لا bucket ~1كم)؛ بلا موقع → cache 10د بمفتاح `home_data:v{N}:...`. (3) `homeCacheVersion()`/`bumpHomeCacheVersion()` + `PlaceObserver::saved()/deleted()` على Place/Event/Store/ZadElgadel → إبطال فوري لـpayloads home. · `app/Helper/App.php`, `app/Http/Controllers/Api/HomeController.php`, `app/Observers/PlaceObserver.php`
- `fix` · [[03_Trips]] [[21_ProfileSettings]] · **`SafeMailTransport` — بريد غير قاتل عالميًا (backend, `7969430`).** decorator يغلّف transport البريد ويبتلع استثناءات الاتصال/الإرسال (يسجّلها)، مسجّل في `AppServiceProvider::boot()` → فشل SMTP لا يكسر HTTP/jobs/notifications. تخفيف جزئي لـ[[KNOWN_ISSUES#K1]]/[[KNOWN_ISSUES#K4]]/[[TECH_DEBT#D45]] (لا يحلّ الجذر: بورت 587 مقفول، لا تسليم، تأخّر 31s). · `app/Mail/SafeMailTransport.php`, `app/Providers/AppServiceProvider.php`
- `docs` · [[02_Home]] [[04_Places]] · مزامنة التوثيق مع `e3cab23`+`7969430`: قواعد/cache/ترتيب home + عقد `getLocationCoords` (params→user_locations→IP) + debounce `update-location` (30م/5د)، تعليم بنود cache-poisoning/staleness مُصلَحة، ADR-01/ADR-02، تحديث K1/K4/D45/API_INDEX. · `FEATURES/02_Home.md`, `FEATURES/04_Places.md`, `KNOWN_ISSUES.md`, `DECISIONS/TECH_DEBT.md`, `DECISIONS/ARCHITECTURE_DECISIONS.md`, `API_INDEX.md`, `PROJECT_PROGRESS.md`
